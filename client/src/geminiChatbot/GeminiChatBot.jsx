...
export default function GeminiChatbot() {
  ...
  return (
    <div className='flex flex-col fixed bottom-5 right-0 md:right-10 mr-2'>
        <div className={ icon ? "w-full max-w-xl mx-auto bg-emerald-800 rounded-2xl p-4 shadow-lg border border-white z-40 " : "hidden"}>
          <div className='w-full flex justify-between items-center'>
            <h2 className="text-xl font-semibold mb-3">AI Portfolio Assistant</h2>
            <button 
              onClick={()=>setIcon(false)}
              className='text-white font-bold hover:scale-110 cursor-pointer'>X</button>
          </div>
          <div
            ref={messagesRef}
          ...
          >
            {messages.map((m) => (
              <div key={m.id} className={m.role === 'user' ? 'text-right' : 'text-left'}>
                <div
                  className={`w-full max-w-1/2 inline-block px-4 py-2 rounded-xl ${
                    m.role === 'user' ? 'bg-amber-500/20' : 'bg-white/10'
                  }`}
                >
                  <p className="text-[12px] sm:text-sm text-white sm:leading-6 whitespace-pre-wrap text-pretty text-start">{m.content}</p>
                </div>
              </div>
            ))}
            {loading && <p className="text-sm text-gray-200 animate-ping">thinking...</p>}
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex gap-2">
            ...
            />
            <button className="px-4 py-2 rounded-xl bg-emerald-500" type="submit" disabled={loading}>
              Send
            </button>
          </form>
        </div>
        ...
        >
          <img
            ...
            alt="Ai icon" 
            className={`${icon ? "w-15 self-end" : "w-15"} sm:w-20 md:w-25 lg:w-30 drop-shadow-emerald-950 drop-shadow-xl`}
            />
            <h3 className={`${icon ? "hidden" : "flex text-sm sm:text-md md:text-lg text-center text-shadow-emerald-950 text-shadow-md" }`}> Talk With <br/>Sami Ai!</h3>
        </div>
    </div>
  );
}